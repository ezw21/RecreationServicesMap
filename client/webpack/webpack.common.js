...
exports.getCommon = function (commonOptions = {}) {
  ...
  const program = require('./parse-command-line').parseCommandLine(commonOptions);
  ...
  const isInPortal = program.portal ? true : false;
  const isDevEdition = program.dev_edition ? true : false;

  // portal and DE build do not use buildNumber
  const buildNumber = (isInPortal || isDevEdition) ? '' : (program.buildNumber || process.env.BUILD_NUMBER || '');
  const jsApiUrl = program.jsApiUrl || process.env.ARCGIS_JSAPI_URL || 'https://js.arcgis.com/4.20/';
      ...
      compiler.hooks.done.tap('CustomPostBuildPlugin-for-StatsWriterPlugin', stats => {
        ...
        if (reportFilename) {
          ...
          if (buildReportFiles.every(f => !!f[1])) {
            ...
            const buildReportData = getBuildReportData(buildReportFiles.map(f => path.resolve(getOutputPath(), f[0])), {});
