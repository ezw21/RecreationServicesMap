...
interface State {
  ...
  filteredVersionList: Array<any>,
  selectedVersion: any,
  selectedService: any,
  selectedWebMap: string,
  vmsServices: Array<any>,
...
export default class Setting extends BaseWidgetSetting<AllWidgetSettingProps<IMConfig>, State>{
          ...
          <Tooltip title={this.formatMessage('layoutStyleBasic')} placement="bottom">
            <div>
              <Button onClick={() => { this.arrangementHandler('simple') }}
                icon size="sm" type="tertiary" active={(this.state.arrangementStyle === 'simple')} style={{ border: (this.state.arrangementStyle === 'simple') ? 'solid #00aabb' : '' }}>
                <Icon width={100} height={100} icon={require('./assets/arrange_basic.svg')} ></Icon>
              </Button>
          ...
          <Tooltip title={this.formatMessage('layoutStyleAdvanced')} placement="bottom">
            <div>
              <Button onClick={() => { this.arrangementHandler('management') }}
                icon size="sm" type="tertiary" active={(this.state.arrangementStyle === 'management')} style={{ border: (this.state.arrangementStyle === 'management') ? 'solid #00aabb' : '' }}>
                <Icon width={100} height={100} icon={require('./assets/arrange_advanced.svg')} ></Icon>
              </Button>
  ...
  renderFieldOrderList = () => {
    ...
    if (Object.keys(this.state.selectedService).length > 0) {
      ...
      fieldsPart.map((fld: any, i: number) => {
        if (fld.available) {
          fieldsList.push(
            <tr key={fld.field}>
              ...
              <td>
                <div style={{ display: 'flex', flexDirection: 'row' }}>
                  {(i > 0) ? <Button icon type="tertiary" size="sm" onClick={() => { this.changeFieldOrder('up', i) }}>
                    <Icon icon={upArrowIcon} size="12" color="#fff" /></Button> : <Button icon type="tertiary" size="sm"></Button>}
                  {(i < fieldsPart.length - 1) ? <Button icon type="tertiary" size="sm" onClick={() => { this.changeFieldOrder('down', i) }}>
                    <Icon icon={downArrowIcon} size="12" color="#fff" /></Button> : <Button icon type="tertiary" size="sm"></Button>}
                </div>
              </td>
            </tr>
          );
    ...
    return (
      ...
      >
        <div style={{ height: '100%', overflow: 'auto' }}>
          ...
          {(this.state.arrangementStyle !== 'simple') ? (this.state.selectedService.hasOwnProperty('name')) &&
        ...
        if (al.enabled) {
          list.push(
            <div key={'refreshInterval'} style={{ paddingBottom: '5px', display: 'flex', flexDirection: 'row', flex: 1 }}>
              <div style={{ display: 'flex', flexDirection: 'row', flex: 4 }}></div>
              <div style={{ textAlign: 'right' }}>
                <Tooltip title={this.formatMessage('settingsSectionsTooltipNumber')} placement="bottom">
                  <TextInput size={'sm'} style={{ width: 40 }} type="text" placeholder="000" value={this.state.intervalNumber}
                    checkValidityOnChange={(value) => {
                      ...
                      if (value.length <= 3) {
                        if (isNaN(newVal)) {
                          ...
                          if (newVal <= 0) {
                            this.setState({ invalidInterval: true, intervalNumber: newVal });
                            valid = false
                          } else {
                            if (value.indexOf('.') > -1) {
                              this.setState({ intervalNumber: +value.replace('.', '') });
                            } else {
                              this.setState({ invalidInterval: false, intervalNumber: newVal }, () => {
                                this.updateAllowanceInterval(al.key, newVal);
                              });
                            }
                        ...
                        if (isNaN(newVal)) {
                          ...
                          if (typeof newVal !== 'number') {
                            intValue = parseInt(newVal.substring(0, 3));
                          }
                          this.setState({ intervalNumber: intValue });
                        }
                      }
                      return { valid }
                    }} />
                </Tooltip>
                <br />{this.formatMessage('settingsSectionsMinutes')}
              </div>
            </div>
          );
        }
  ...
  renderServiceSelector = () => {
    ...
    if (this.state.vmsServices.length > 0) {
      this.state.vmsServices.map((s: any, idx: number) => {
        const tableRow = <tr key={s.name} style={{ borderLeft: (s.name === this.state.selectedService.name) ? 'solid #00aabb' : '' }} >
          {
            ...
            <td>
              ...
              {(idx < this.state.vmsServices.length - 1) ? <Button icon type="tertiary" size="sm" onClick={() => { this.changeServiceOrder('down', idx) }}>
  ...
  validateVMSExist = async (evt: any) => {
    ...
    for (const key in dsList) {
      if (key === evt[0].dataSourceId) {
        ...
        if (dsJson.url.indexOf('FeatureServer') >= 0) {
          trunURL = dsJson.url.substring(0, dsJson.url.indexOf('FeatureServer'));
        }
        if (dsJson.url.indexOf('MapServer') >= 0) {
  ...
  renderVersionSelector = () => {
    ...
    this.state.filteredVersionList.map((v: any) => {
    ...
    return (
      <React.Fragment>
        <SettingSection className="map-selector-section" title={this.formatMessage('settingsSectionsGeneralSettings')}>
          ...
          <SettingRow>
            <Select onChange={(evt: any) => { this.updateGeneralItem(evt.currentTarget.value, 'pageCounter') }} defaultValue={defaultVal}>
              {pageValues}
            </Select>
          </SettingRow>
        </SettingSection>
