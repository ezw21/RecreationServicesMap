...
export default class GDBVersionManager {
  ...
  private defaultVersion: string = 'sde.DEFAULT';
  private isOrgUser: boolean = false;
  private uniqueURLs: Array<any> = [];

  getUniqueVMSURL(specific?: string): Promise<Array<string>> {
  ...
  async checkValidVMS(url: string, token?:string): Promise<boolean> {
    let returnVal = false;
    let trunURL = url;
    if (trunURL.indexOf('FeatureServer') >= 0) {
      trunURL = trunURL.substring(0, trunURL.indexOf('FeatureServer'));
    }
    if (trunURL.indexOf('MapServer') >= 0) {
    ...
    return new Promise((resolve, reject) => {
      if (url) {
        ...
        if (this.uniqueURLs.length == 0) {
          this.getUniqueVMSURL().then((arrResult: Array<any>) => {
  ...
  changeGDBVersion(name: string, dsList: Array<any>): boolean {
    let returnVal = false;
    for (const key in dsList) {
      if (dsList[key].type == DataSourceTypes.FeatureLayer) {
  ...
  createGDBVersion(versionObj: any, token: string, dsList: Array<any>, service?: string, url?: string, autoSwitch?: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      const returnVal = {};
      let requestURL = null;
      if (this.isOrgUser) {
  ...
  deleteGDBVersion(name: string, token: string, dsList: Array<any>, service?: string, url?: string): Promise<any> {
    return new Promise((resolve, reject) => {
  ...
  alterGDBVersion(versionObj: any, token: string, dsList: Array<any>, service?: string, url?: string, autoSwitch?: boolean): Promise<any> {
    return new Promise((resolve, reject) => {
      const returnVal = {};
      let requestURL = null;
      if (this.isOrgUser) {
  ...
  searchPortalUsers(url: string, search: string, token: string): any {
    return new Promise((resolve, reject) => {
      ...
      const userURL = url + '/sharing/rest/community/users';
      this.requestService({
        method: 'POST', url: userURL,
        params: {
          f: 'json',
          q: search,
          token: token
