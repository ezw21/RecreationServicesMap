...
export default class AddressSettings extends React.PureComponent<Props, State>{
  private geocodeTextBox = React.createRef<HTMLInputElement>();
  constructor(props) {
    super(props);
  ...
  onSetLocatorClicked = () => {
    ...
    setTimeout(() => {
      ...
      this.geocodeTextBox.current.value = currentValue;
    }, 500);
  }
    ...
    return <div style={{ height: '100%', marginTop: '5px' }}>
      <div css={getAddressSettingsStyle(this.props.theme)}>
        ...
        <SettingRow label={this.nls('minCandidateScore')}>
          <NumericInput aria-label={this.nls('minCandidateScore')} className={'candidateScoreInput'}
            min={0} max={100} defaultValue={this.state.candidateScoreInput}
            onChange={this.onCandidateScoreChange} />
        </SettingRow>

        {/* Popup dialog for selecting the valid geocode service */}
        <AlertPopup isOpen={this.state.isAlertPopupOpen} css={getAlertPopupStyle(this.props.theme)}
          ...
          <div className={'popupContents'}>
            <div className={'alertValidationContent'}>
              <TextInput role={'textbox'} aria-label={this.state.geocodeLocatorUrl} required
                ...
                type='text' ref={this.geocodeTextBox}
                value={this.geocodeTextBox?.current?.value ? this.geocodeTextBox.current.value : this.state.geocodeLocatorUrl}
