...
import { NumericInput, AlertPopup, Label } from 'jimu-ui';
    ...
    return <div style={{ height: '100%', marginTop: '5px' }}>
      <div css={getGeneralSettingsStyle(this.props.theme)}>
        <SettingRow label={this.nls('locationSymbol')} flow={'wrap'}>
          <div className={'symbolItem'} onClick={this.showSymbolChooser.bind(this)}>
            <SymbolItem symbol={this.state.defaultSymbol} isUsedForPreview></SymbolItem>
          </div>
        </SettingRow>
        ...
        <SettingRow label={this.nls('zoomScale')}>
          <Label>1 :</Label>
          <NumericInput aria-label={this.nls('zoomScale')} className={'zoomScaleInput'} defaultValue={this.state.zoomScaleInput}
            showHandlers={false} onChange={this.onZoomScaleInputChange} />
        </SettingRow>

        {/* Popup dialog for symbol selector */}
        <AlertPopup isOpen={this.state.isPanelOpened}
          onClickOk={this.onAlertOKButtonClicked.bind(this)} onClickClose={this.closeSymbolChooser}
          title={this.props.intl ? this.nls('locationSymbol') : ''}>
          <SymbolChooser
            intl={this.props.intl}
            symbol={this.state.defaultSymbol}
            onPointSymbolChanged={this.onSymbolChange.bind(this)}
          />
        </AlertPopup>
      </div>
    </div>
