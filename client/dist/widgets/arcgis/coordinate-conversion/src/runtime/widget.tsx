...
import { jsx, AllWidgetProps, BaseWidget, getAppStore, ImmutableArray } from 'jimu-core';
...
export default class Widget extends BaseWidget<AllWidgetProps<IMConfig>, State> {
  ...
  constructor(props) {
    ...
    if (this.props.config?.addressSettings?.geocodeServiceUrl) {
      ...
      this.props.portalSelf.helperServices.geocode.length > 0 && this.props.portalSelf.helperServices.geocode[0].url) {
      ...
      cVM.watch('messages', () => {
        if (this._isMounted) {
          setTimeout(() => {
            ...
            const newOutputDisplayInfos = [], outputDisplayInfosToBeAppended = [];
            let append = false;
              ...
              this._outputDisplayInfos.forEach((outputDisplayInfo) => {
                ...
                if (outputDisplayInfo.addedAtRuntime) {
                  if (configuredSetting.name === 'address') {
                    ...
                    const defaultFormat = generateDefaultFormats(cVM.messages).filter((format) => {
              ...
              if (outputDisplayInfosToBeAppended && outputDisplayInfosToBeAppended.length > 0) {
                outputDisplayInfosToBeAppended.forEach((outputDisplayInfo) => {
                  ...
                  this._outputDisplayInfos.push({
                    displayLabel: outputDisplayInfo.displayLabel,
                    isOpen: outputDisplayInfo.isOpen, format: newConversion.format, addedAtRuntime: true
  ...
  renderOutputList = (results) => {
    ...
    results.map((conversionInfo, index) => {
  ...
  addNewFormat = (name, label, currentPattern, addAtTheTop) => {
    ...
    if (name === 'address') {
      ...
      const newFormats = generateDefaultFormats(this.state.outputFormatCoordVM.messages);
