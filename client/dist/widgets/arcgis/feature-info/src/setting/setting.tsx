...
import {
  ...
  UseDataSource, SqlExpression, IMThemeVariables, OrderByOption, DataSourceComponent, QueriableDataSource
} from 'jimu-core'
import { Button, ButtonGroup, Dropdown, DropdownButton, DropdownMenu, DropdownItem, Checkbox, TextArea, UnitTypes, LinearUnit, Select } from 'jimu-ui'
  ...
  render () {
    ...
    if (this.props.config.styleType === 'custom') {
      if (this.props.config.style.fontSizeType === FontSizeType.custom) {
        unitContent = (
          <InputUnit
            style={{ width: '6.5rem' }} units={[UnitTypes.PIXEL, UnitTypes.PERCENTAGE]} className='item' min={1} value={this.getStyleConfig().fontSize as LinearUnit}
      ...
      styleContent = (
        <div>
          ...
          <SettingRow label={<FormattedMessage id='textColor' defaultMessage='Text color' />}>
            <ThemeColorPicker specificTheme={this.props.appTheme} value={this.getStyleConfig().textColor} onChange={value => this.onStyleChanged('textColor', value)} />
          </SettingRow>
        </div>
      )
    }
