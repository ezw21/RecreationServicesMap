...
import { Button, Icon, Slider, Dropdown, DropdownButton, DropdownMenu, DropdownItem, defaultMessages as jimuUiNls, NumericInput, Progress } from 'jimu-ui'
...
// fly facade
import FlyManager, { UiLoadingCallbacks } from '../../common/fly-facade/fly-manager'
...
const routesIconImage = require('../../assets/icons/fly-routes.svg')
...
export default class InteractivePanel extends React.PureComponent<Props, State> {
  ...
  resetDefaultUI = (actions: { isCleanGraphics?: boolean }): void => {
    ...
    // interaction
    this.graphicInteractionManagerRef?.onRenderStateChange({ drawingMode: DrawingMode.Null, isPicking: false })
    // controller
    this.handleStop()
    ...
    if (actions?.isCleanGraphics) {
      ...
      this.graphicInteractionManagerRef?.clearAll()
    }
  }
  ...
  setPlayStatePlaying = (playState: boolean): void => {
    ...
    this.graphicInteractionManagerRef?.onRenderStateChange(null)
  }
  ...
  getFlyStyleContent = (): React.ReactElement => {
    ...
    if (flyStyle === FlyItemMode.Rotate) {
      ...
      flyStyleContent = <Icon icon={routesIconImage} />
    }
  ...
  isShowDropdownButtonArrow = (layout: PanelLayout): boolean => {
    ...
    if (this.flyManager.itemsList?.length > 1) {
      return true
    }
    return false
  }
    ...
    return (
      <Dropdown isOpen={this.state.isFlyStylePopupOpen} toggle={this.toggleFlyStylePopup} className='dropdowns flystyle-btn' activeIcon>
        ...
        <DropdownMenu showArrow={false}>
          {this.flyManager.itemsList.map((itemConfig, idx) => {
  ...
  renderRouteFlyModeContent (layout: PanelLayout): React.ReactElement {
    ...
    return (
      <RouteMenu
        ...
        layout={layout}
        isEnable={isEnable}
        itemsList={this.flyManager.itemsList}
