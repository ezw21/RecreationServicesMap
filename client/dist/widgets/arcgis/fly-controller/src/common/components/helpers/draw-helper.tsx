...
export default class DrawHelper extends React.PureComponent<Props, States> {
  ...
  drawingMode: DrawingMode
  jimuDrawToolsRef: DrawToolClass
      ...
      this.props.layersGroup.layerOnTheGround && this.props.layersGroup.layerRelativeToGround &&
        ...
        <div style={{ display: 'none' }}>
          <Draw
            ...
            onDrawToolCreated={this.handleDrawToolCreated}
            onDrawToolsActived={this.handleDrawToolsActived}
            onDrawStart={this.handleDrawStart}
            onDrawEnd={this.handleDrawEnd}
            onDrawCancel={this.handleDrawCancel}
            onGraphicEdited={this.handleDrawFinish}
            // visibleElements
  ...
  handleDrawToolCreated = async (jimuDrawToolsRef: DrawToolClass): Promise<any> => {
    this.jimuDrawToolsRef = jimuDrawToolsRef

    await this.jimuDrawToolsRef.complete()
  ...
  // return jsObjects
  handleDrawFinish = ({ type, graphic, layer }: { type: ('complete' | 'deleted' | 'modified' | 'aborted'), graphic: __esri.Graphic, layer: __esri.GraphicsLayer }): void => {
    if (type === 'deleted' || type === 'modified') {
      return // continue the workflow
    }

    layer?.removeAll()// clean drawinglayer
  ...
  getGraphicsByAttrId = (attrId: string): __esri.Graphic[] => {
    let res = []
    const mainGraphic = this.props.layersGroup.layerOnTheGround.graphics.toArray().filter(graphics => (graphics.attributes.id === attrId))
    if (mainGraphic.length > 0) {
      // 1. clamp ground mode
      res.push(mainGraphic[0])
      res = res.concat(this.props.layersGroup.layerRelativeToGround.graphics.toArray().filter(graphics => (graphics.attributes.id === attrId)))
      return res
    } else {
      // 2. on the ground mode
      return this.props.layersGroup.layerRelativeToGround.graphics.toArray().filter(graphics => (graphics.attributes.id === attrId))
    }
  }
  ...
  _removeGraphicsByAttrId = (rawGraphic: __esri.Graphic): void => {
    let duplicatedGraphics = this.props.layersGroup.layerRelativeToGround.graphics.toArray().filter(graphics => (graphics.attributes.id === rawGraphic.attributes.id))
    if (duplicatedGraphics.length > 0) {
      this.props.layersGroup.layerRelativeToGround.removeMany(duplicatedGraphics)
    }

    duplicatedGraphics = this.props.layersGroup.layerOnTheGround.graphics.toArray().filter(graphics => (graphics.attributes.id === rawGraphic.attributes.id))
    if (duplicatedGraphics.length > 0) {
  ...
  private readonly genExtraPoint = (p, graphic: __esri.Graphic, symbol: __esri.PointSymbol3D): __esri.Graphic => {
    ...
    pt = new this.Graphic({
      ...
      layer: this.props.layersGroup.layerRelativeToGround,
      attributes: {
        id: graphic.attributes.id// same id as graphic
      }
    })
    return pt
  }
  ...
  private readonly updateLineByAltitude = (graphic: __esri.Graphic, alt: number): __esri.Graphic[] => {
    ...
    const graphics = this.getGraphicsByAttrId(graphic.attributes.id)
