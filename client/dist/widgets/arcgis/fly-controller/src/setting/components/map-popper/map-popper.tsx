...
const closeIcon = require('jimu-ui/lib/icons/close-12.svg')
...
export class MapPopper extends React.PureComponent<Props, States> {
            ...
            <div className='popper-footer'>
              <div className='left-tool'>
                <ActionPanel
                  ...
                  onRefGraphicInteractionManager={this.props.onRefGraphicInteractionManager}
                  //
  ...
  _toggleConfirmWindow = (cmd?: boolean): void => {
    ...
    this.setState({ isShowConfirmWin: isShowConfirmWin }, () => {
      ...
      >
        <div className='real-container'>
          ...
          <div className='confirm-context'>
            <TextInput className='w-100' defaultValue={this.getRecordName() ?? ''} onChange={evt => this.handleConfirmWinNameChange(evt.target.value)} />
          </div>
          <div className='confirm-footer'>
            <Button type='secondary' onClick={this.handleConfirmWinEdit}>{editElementLabel}</Button>
            <Button type='primary' onClick={this.handleConfirmWinSave}>{saveLabel}</Button>
          </div>
        </div>
      </div>
    )
