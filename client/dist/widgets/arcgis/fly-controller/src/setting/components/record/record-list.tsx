...
const recordStopIcon = require('../../../assets/icons/record-stop.svg')
...
export class RecordList extends React.PureComponent<Props, States> {
  ...
  mapRoutesToListItems = (records: RecordConfig[]): TreeItemsType => {
    ...
    const listItems = records?.map((record: RecordConfig/*, keyIdx */) => {
      ...
      if (record.type === ControllerMode.Rotate) {
        headIcon = { icon: require('../../../assets/icons/mark-point.svg') } // title={pointStyleLabel}
      } else if (record.type === ControllerMode.Smoothed || record.type === ControllerMode.RealPath) {
        headIcon = { icon: require('../../../assets/icons/trigger-draw-line.svg') } // title={pathStyleLabel}
      }
      ...
      return {
        ...
        itemStateCommands: [
          { // [0]
            label: editLabel,
            iconProps: () => ({ icon: require('jimu-ui/lib/icons/tool-edit.svg') }),
            action: (data) => {
            ...
            label: previewLabel,
            collapsed: false,
            iconProps: () => ({ icon: require('jimu-ui/lib/icons/app-launch.svg') }),
            action: (data) => {
            ...
            label: stopLabel,
            collapsed: true,
            iconProps: () => ({ icon: recordStopIcon }),
            action: (data) => {
              this.handlePauseRecordPreview(data.data.itemJsons[0].itemKey)
            }
          }, { // [3]
            label: deleteLabel,
            iconProps: () => ({ icon: require('jimu-ui/lib/icons/tool-delete.svg') }),
            action: (data) => {
  ...
  updateListItemsByPlayingInfo = (): void => {
    const listItems: ListItemsType = this.state.listItems?.map((item: TreeItemType/*, keyIdx */) => {
      ...
      if (isPlaying) {
        itemStateCommands[1].collapsed = true// app-launch.svg
        itemStateCommands[2].collapsed = false// record-stop.svg
      } else {
