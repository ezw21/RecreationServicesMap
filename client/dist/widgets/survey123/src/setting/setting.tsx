...
// the style of dropdown menu of survey list
const surveyListMenuProps = {
  style: {
    maxWidth: 228
  }
};
...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<IMConfig>, any>{
  ...
  constructor(props: any) {
    ...
    this.isOrgAdmin = (user && user.role === 'org_admin') ?  true : false;
    this.prepare().then(() => {
      this.querySurvey();
  ...
  createSurvey = () => {
     ...
     */
     ...
     let tags = null;
     if (this.state.newSurveyTags && (this.state.newSurveyTags + '').trim()) {
       tags = (this.state.newSurveyTags || '').split(',').map((tag) => tag.trim());
     }
        ...
        <div className="survey123__section-createSurvey" style={
          ...
          <SettingSection>
            ...
            <div className="w-100 d-flex flex-wrap align-items-center justify-content-between setting-header setting-title pb-2">
              ...
              <TextInput className="w-100" value={this.state.newSurveySnippet || ''} onChange={(evt) => this.onNewSurveyValueChanged(evt, 'newSurveySnippet')} />
            </div>
