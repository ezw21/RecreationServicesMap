...
import { SingleLineStyle } from './single-line-style'
...
export const SerialLineElements = Object.values(LineElementPathes)
...
export const LineStyle = (props: LineStyleProps): React.ReactElement => {
  const { className, elements = SerialLineElements, webChart, onChange } = props
  ...
  const handleAllElementsChange = (key: string, value: any): void => {
    let ret = webChart
      ...
      <SettingSection>
        {useAll && (
          <LineSymbolSetting
            ...
            value={allSymbol}
            onPropsChange={handleAllElementsChange}
          />
        )}
        {!useAll && (
          <>
            {elements.map((path, idx) => {
              ...
              return (
                <SingleLineStyle
                  ...
                  key={idx}
                  value={symbol}
                  onChange={value => handleSingleChange(path, value)}
                />
              )
            })}
          </>
