...
const IconImage = require('jimu-ui/lib/icons/arrow-left-16.svg');
...
export class BackBtn extends React.PureComponent<Props> implements IBackable {
  ...
  render() {
    ...
    if (this.props.uiMode === UiMode.Inline) {
      ...
      content = (
        <Button onClick={evt => this.onBackBtnClick(evt)} size='sm' css={this.getStyle()} icon>
          <Icon icon={IconImage} options={{ currentColor: false }} width={20} height={20} autoFlip />
        </Button>
      )
    }
