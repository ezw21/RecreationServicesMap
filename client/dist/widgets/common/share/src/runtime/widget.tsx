...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & ExtraProps, State> {
  ...
  private readonly POPPER_POSITION_CONFIGS = [getFallbackPlacementsModifier(['right-start', 'left-start', 'bottom'])]
  ...
  private readonly renderOutsideUI = () => {
    ...
    if (uiMode === UiMode.Popup) {
      if (this.state.isInController) {
        ...
        outsideUI = (
          <Button ref={this.btnRef} icon onClick={this.onPopupBtnClick} title={tooltip} style={{ border: 'none', backgroundColor: 'transparent', padding: 0 }} data-testid='popupBtn'>
  ...
  // part 2
  private readonly renderPopupTitle = () => {
